...
public:
    ...
    // Compute total distance along a given journey path
    double computeTotalDistance(const vector<string> &journey)
    {
        double total = 0.0;
        for(size_t i = 1; i < journey.size(); ++i)
        {
            const string &from = journey[i-1];
            const string &to   = journey[i];

            auto it = line.find(from);
    ...
    void cheapestRoute(string src, string dest)
    {
        ...
        // Calculate distance and time for this journey
        double totalDist = computeTotalDistance(journey);
        const double avgSpeedKmph = 30.0; // assumed average metro speed
        double totalMinutes = (totalDist / avgSpeedKmph) * 60.0;

        cout<<endl<<endl;
        cout<<"*** Total cost of journey will be rupees "<<visitedCost[dest]<<" ***"<<endl;
        cout<<"*** Total distance of journey will be "<<fixed<<setprecision(2)<<totalDist<<" kms ***"<<endl;
        cout<<"*** Estimated travel time is "<<fixed<<setprecision(1)<<totalMinutes<<" minutes ***"<<endl<<endl<<endl;
    }
    ...
    void ShortestRoute(string src, string dest)
    {
        ...
        double totalDist = visitedDist[dest];
        const double avgSpeedKmph = 30.0; // assumed average metro speed
        double totalMinutes = (totalDist / avgSpeedKmph) * 60.0;
